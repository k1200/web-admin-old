import {
  fn_api__system_logininfor_list,
  fn_api__system_logininfor_del,
  fn_api__system_logininfor_clean
...
export default {
  ...
  methods: {
    ...
    async fn_handle__get_list(form = this.query_params) {
      this.loading = true;
      new Promise(resolve => {
        fn_api__system_logininfor_list(this.fn_format__reqdata(form))
          .then(res => {
    ...
    fn_click__del(ids) {
      if (Object.prototype.toString.call(ids) === '[object Array]') {
        ids = ids.map(item => item.infoId).toString();
      }
      if (ids) {
        this.$confirm(`此操作将删除用户编号为${ids}的数据, 是否继续?`, '提示', {
          ...
          .then(() => {
            fn_api__system_logininfor_del(ids).then(res => {
              if (res.code === 200) {
                this.fn_handle__get_list();
    ...
    fn_click__clearn() {
      this.$confirm(`此操作将清空所有登陆日志的数据, 是否继续?`, '提示', {
        ...
        .then(() => {
          fn_api__system_logininfor_clean().then(res => {
            if (res.code === 200) {
              this.fn_handle__get_list();
              this.$message.success('删除成功!');
            }
